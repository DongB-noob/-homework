...
if __name__=='__main__':
    ...
    '''        VGG系列    '''
    # model = vgg11(pretrained=False, num_classes=5).to(device)   # 43.4%
    # model = vgg13(pretrained=False, num_classes=5).to(device)   # 34.8%
    # model = vgg16(pretrained=False, num_classes=5).to(device)   # 37.0%


    '''      ResNet系列    '''
    # model = resnet18(pretrained=False, num_classes=5).to(device)    # 57.3%
    # model = resnet34(pretrained=False, num_classes=5).to(device)    # 53.1%
    # model = resnet50(pretrained=False, num_classes=5).to(device)    # 31.1%
    model = resnet101(pretrained=False, num_classes=5).to(device)   # 31.7%
